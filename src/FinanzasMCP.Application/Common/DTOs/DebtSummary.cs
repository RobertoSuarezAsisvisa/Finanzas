...
public sealed record DebtSummary(Guid Id, DebtType Type, string ContactName, decimal OriginalAmount, decimal RemainingAmount, string Currency, DateTimeOffset? DueDate, Guid? AccountId, DebtStatus Status, string? Notes, decimal? InterestRate, InterestPeriod? InterestPeriod, AmortizationMethod? AmortizationMethod, int? TermMonths, DateTimeOffset? LoanStartDate);
